...
import be.helha.labos.crystalclash.User.*;
...
public class LanternaApp {
    ...
    private static void afficherMenuPrincipal(WindowBasedTextGUI gui) {
        ...
        mainPanel.addComponent(new Button("4. Se déconnecter", () -> {
            Session.clear();
    ...
    private static void afficherInventaire(WindowBasedTextGUI gui) {
        ...
        try {
            ...
            if (jsonObject.has("inventory")) {
                Gson gson = new Gson();
                ObjectBase[] objets = gson.fromJson(jsonObject.get("inventory"), ObjectBase[].class);
